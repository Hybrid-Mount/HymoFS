 ...
 * ======================================================================
 * Provides fallback implementations for dentry_path_raw, d_absolute_path,
 * and d_hash_and_lookup when these kernel symbols are not exported
 * (e.g., on some GKI kernels).
 */

static struct dentry *(*hymo_d_lookup_func)(struct dentry *, struct qstr *);

static inline char *hymo_prepend_name(char *buffer, int *buflen, const struct qstr *name)
{
...
static char *hymo___dentry_path(const struct dentry *dentry, char *buf, int buflen)
{
	...
	*p = '/';
	...
	while (d && d->d_parent != d) {
		...
		p = hymo_prepend_name(buf, &buflen, name);
		if (!p) {
			error = -ENAMETOOLONG;
			break;
		}
...
static char *hymo_dentry_path_raw_fallback(const struct dentry *dentry, char *buf, int buflen)
{
	...
	rcu_read_lock();
	res = hymo___dentry_path(dentry, buf, buflen);
	rcu_read_unlock();
